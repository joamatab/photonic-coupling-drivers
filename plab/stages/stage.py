...
class Stages3(Stages, metaclass=abc.ABCMeta):
    ...
    def _ctr_in_out_x_axes(self, x_ctr_around_line_um=None):
        if x_ctr_around_line_um is None:
            x_ctr_around_line_um = (
                self.input.x.get_position_absolute_max_um()
                - self.input.x.get_position_absolute_min_um()
            ) / 2.0
    ...
    def _ctr_in_out_y_axes(self, y_ctr_around_line_um=None):
        if y_ctr_around_line_um is None:
            y_ctr_around_line_um = (
                self.input.y.get_position_absolute_max_um()
                - self.input.y.get_position_absolute_min_um()
            ) / 2.0
    ...
    def ctr_in_out_x_axes(self, x_ctr_around_line_um=None):
        ...
        # Pull fibres away.
        self.input.z.move_rel_um(-20.0)
        self.output.z.move_rel_um(-20.0)
    ...
    def move_rel_um_x_long(
        self, move_rel_x_um, move_rel_output_x_um=None, x_ctr_around_line_um=None
    ):
        """
        ...
        assert (
            abs(move_rel_x_um - move_rel_output_x_um)
            < self.input.x.get_position_absolute_max_um()
    ...
    def move_rel_um_c_long(
        self, move_rel_c_um, move_rel_output_c_um=None, c_ctr_around_line_um=None
    ):
        """
        ...
        x_rel_in_um, y_rel_in_um, z_rel_in_um = self.input.c._get_xyz_rel_move_um(
            move_rel_c_um
        )
        x_rel_out_um, y_rel_out_um, z_rel_out_um = self.output.c._get_xyz_rel_move_um(
            move_rel_output_c_um
        )
        ...
        # Move chip
        xi, xo, c = self.move_rel_um_x_long(
            x_rel_in_um, x_rel_out_um, c_ctr_around_line_um
        )
    ...
    def move_rel_um_xc_long(
        self, move_rel_xc_um, move_rel_output_xc_um=None, xc_ctr_around_line_um=None
    ):
        """
...
class Stage(object, metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        self.axes["x"], self.axes["y"], self.axes["z"] = (
            self.axes[x_axis_motor],
            self.axes[y_axis_motor],
            self.axes[z_axis_motor],
        )
    ...
    def get_current_position_um(self):
        return tuple(
            v / 1000.0 if v != None else None for v in self.get_current_position_nm()
        )

    def get_current_position_degree(self):
        return tuple(
            v / 3600.0 if v != None else None
    ...
    def set_c_axis(self, C1, C2, C1_z_chip, C2_z_chip, c1_c2_distance_mask_um=None):
        ...
        self.axes["c"] = AxisChip(
            self.axes["x"],
            self.axes["y"],
            self.axes["z"],
            C1,
            C2,
            c1_c2_distance_mask_um,
        )
...
class Axis(object, metaclass=abc.ABCMeta):
    ...
    def _update_position_absolute(self):
        if self._update_position_absolute:
            if (
                self._update_position_absolute_counter
                >= self._update_position_absolute_limit
            ):
...
class AxisLinear(Axis, metaclass=abc.ABCMeta):
    ...
    def _move_abs(self, distance_from_home_nm):
        if self.axis_reversed:
            distance_from_home_nm = (
                self._position_absolute_max_nm - distance_from_home_nm
            )
        ...
        if distance_from_home_nm == self._position_absolute:
            ...
            self._position_absolute = self.position_absolute_within_bounds(
                self._position_absolute
            )
    ...
    def position_absolute_within_bounds(self, position_absolute):
        ...
        if position_absolute < self.get_position_absolute_min_nm():
            raise ValueError(
                "%s-axis target movement `%.3f` [um] is too small."
                % (self.name, position_absolute / 1000.0)
            )
        if position_absolute > self.get_position_absolute_max_nm():
            raise ValueError(
                "%s-axis target movement `%.3f` [um] is too large."
                % (self.name, position_absolute / 1000.0)
            )
    ...
    def move_abs_um(self, distance_from_home_um):
        ...
        """
        return self._move_abs(distance_from_home_um * 1000.0) / 1000.0
    ...
    def move_rel_um(self, distance_um):
        ...
        """
        r = self.move_rel_nm(distance_um * 1000.0)
        return r / 1000.0
    ...
    def get_current_position_um(self):
        ...
        """
        return self.get_current_position_nm() / 1000.0
    ...
    def get_absolute_position_start_um(self):
        ...
        """
        return self._position_absolute_start / 1000.0
    ...
    def get_position_absolute_min_um(self):
        ...
        """
        return self._position_absolute_min_nm / 1000.0
    ...
    def get_position_absolute_max_um(self):
        ...
        """
        return self._position_absolute_max_nm / 1000.0
...
class AxisRotate(Axis, metaclass=abc.ABCMeta):
    ...
    def _move_abs(self, angle_from_home_arc_second):
        if self.axis_reversed:
            angle_from_home_arc_second = (
                self._position_absolute_max_arc_second - angle_from_home_arc_second
            )
    ...
    def _move_rel(self, angle_arc_second):
        if self.axis_reversed:
            angle_arc_second *= -1.0

        if angle_arc_second == 0.0:
            # Don't issue a move or log if the position hasn't changed.
            r = angle_arc_second
        else:
    ...
    def position_absolute_within_bounds(self, position_absolute):
        ...
        if position_absolute < self.get_position_absolute_min_arc_second():
            raise ValueError(
                "%s-axis target movement `%.3f` [degree] is too small."
                % (self.name, position_absolute / 3600.0)
            )
        if position_absolute > self.get_position_absolute_max_arc_second():
            raise ValueError(
                "%s-axis target movement `%.3f` [degree] is too large."
                % (self.name, position_absolute / 3600.0)
            )
        return position_absolute
    ...
    def move_abs_degree(self, angle_from_home_degree):
        ...
        """
        return self._move_abs(angle_from_home_degree * 3600.0) / 3600.0
    ...
    def move_rel_degree(self, angle_degree):
        ...
        """
        return self._move_rel(angle_degree * 3600.0) / 3600.0
    ...
    def get_current_position_arc_second(self):
        ...
        if self.axis_reversed:
            pos_abs_arc_second = (
                self._position_absolute_max_arc_second - pos_abs_arc_second
            )
    ...
    def get_current_position_degree(self):
        ...
        """
        return self.get_current_position_arc_second() / 3600.0
    ...
    def get_absolute_position_start_degree(self):
        ...
        """
        return self._position_absolute_start / 3600.0
    ...
    def get_position_absolute_min_degree(self):
        ...
        """
        return self._position_absolute_min_arc_second / 3600.0
    ...
    def get_position_absolute_max_degree(self):
        ...
        """
        return self._position_absolute_max_arc_second / 3600.0
...
class AxisChip(object):
    ...
    def _get_xyz_rel_move_nm(self, rel_move_nm):
        c = self._get_xyz_rel_move_um(rel_move_nm / 1000.0)
        return tuple(v * 1000.0 for v in c)
    ...
    def move_rel_nm(self, distance_rel_nm):
        return self.move_rel_um(distance_rel_nm / 1000.0)
    ...
    def get_current_position_um(self):
        ...
        return tuple(p / 1000.0 for p in pos)

    def get_distance_c1_to_c2_nm(self):
        return self.get_distance_c1_to_c2_um() * 1000.0
