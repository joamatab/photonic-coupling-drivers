...
class GenericStage:
    def __init__(self, port, chan_ident, ini_section):
        ...
        self._config.read_string(
            pkgutil.get_data("thorpy.stages", "MG17APTServer.ini").decode("ascii")
        )
        ...
        self._conf_home_limit_switch = self._config.getint(
            ini_section, "Home Limit Switch"
        )
        self._conf_home_vel = self._config.getfloat(ini_section, "Home Vel")
        self._conf_home_zero_offset = self._config.getfloat(
            ini_section, "Home Zero Offset"
        )
        ...
        self._conf_dc_servo = self._config.getboolean(
            ini_section, "DC Servo", fallback=False
        )
        ...
        self._conf_fp_controls = self._config.getboolean(
            ini_section, "FP Controls", fallback=False
        )
        ...
        self._conf_js_params = self._config.getboolean(
            ini_section, "JS Params", fallback=False
        )
        if self._conf_js_params:
            self._conf_js_gearlow_maxvel = self._config.getfloat(
                ini_section, "JS GearLow MaxVel"
            )
            self._conf_js_gearlow_accn = self._config.getfloat(
                ini_section, "JS GearLow Accn"
            )
        ...
        self._port.send_message(
            MGMSG_MOD_SET_CHANENABLESTATE(
                chan_ident=self._chan_ident, chan_enable_state=0x01
            )
        )
    ...
    def _handle_message(self, msg):
        ...
        if (
            isinstance(msg, MGMSG_MOT_GET_DCSTATUSUPDATE)
            or isinstance(msg, MGMSG_MOT_GET_STATUSUPDATE)
            or isinstance(msg, MGMSG_MOT_MOVE_COMPLETED)
        ):
    ...
    @property
    def position(self):
        self._wait_for_properties(
            ("_state_position",),
            timeout=3,
            message=MGMSG_MOT_REQ_DCSTATUSUPDATE(chan_ident=self._chan_ident),
        )
    ...
    def position(self, new_value):
        ...
        self._port.send_message(
            MGMSG_MOT_MOVE_ABSOLUTE_long(
                chan_ident=self._chan_ident, absolute_distance=absolute_distance
            )
        )
    ...
    def velocity(self):
        ...
        return self._state_velocity / (
            self._EncCnt * self._T
        )  # Dropped the 65536 factor, which resulted in false results
    ...
    def min_velocity(self):
        self._wait_for_properties(
            ("_state_min_velocity",),
            timeout=3,
            message=MGMSG_MOT_REQ_VELPARAMS(chan_ident=self._chan_ident),
        )
    ...
    def home_velocity(self):
        self._wait_for_properties(
            ("_state_home_velocity",),
            timeout=3,
            message=MGMSG_MOT_REQ_HOMEPARAMS(chan_ident=self._chan_ident),
        )
        return self._state_home_velocity / (self._EncCnt * self._T * 65536)

    @home_velocity.setter
    def home_velocity(self, new_value):
        self._set_homeparams(
            float(new_value),
            self.home_direction,
            self.home_limit_switch,
            self.home_offset_distance,
        )

    @property
    def home_direction(self):
        self._wait_for_properties(
            ("_state_home_direction",),
            timeout=3,
            message=MGMSG_MOT_REQ_HOMEPARAMS(chan_ident=self._chan_ident),
        )
    ...
    def _set_homeparams(
        self, home_velocity, home_direction, home_limit_switch, home_offset_distance
    ):
    ...
    def print_state(self):
        ...
        if (
            self.status_in_motion_forward
            or self.status_in_motion_reverse
            or self.status_in_motion_jogging_forward
            or self.status_in_motion_jogging_reverse
            or self.status_in_motion_homing
        ):
        ...
        print("")
        print(
            "Velocity parameters: velocity: {0.min_velocity:0.3f}-{0.max_velocity:0.3f}{0.units}/s, acceleration: {0.acceleration:0.3f}{0.units}/s²".format(
                self
            )
        )
        print(
            "Homing parameters: velocity: {0.home_velocity:0.3f}{0.units}/s, direction: {0.home_direction}, limit_switch: {0.home_limit_switch}, offset_distance: {0.home_offset_distance:0.3f}{0.units}".format(
                self
            )
        )
    ...
    def home(self, force=False):
        ...
        while not self.status_homed:
            if not self.status_in_motion_forward and not self.status_in_motion_reverse:
                self._port.send_message(
                    MGMSG_MOT_MOVE_HOME(chan_ident=self._chan_ident)
                )
            time.sleep(1)
    ...
    def _wait_for_properties(
        self, properties, timeout=None, message=None, message_repeat_timeout=None
    ):
    ...
    def __repr__(self):
        return "<{0} on {1!r} channel {2}>".format(
            self._name, self._port, self._chan_ident
        )
