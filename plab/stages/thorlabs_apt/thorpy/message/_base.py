...
class Message:
    ...
    def __init__(self, *args, source=0x01, dest=None, **kwargs):
        ...
        parameter_mapping = {
            name: position for position, (name, encoding) in enumerate(self.parameters)
        }
        for name, value in kwargs.items():
            try:
                ...
                raise KeyError(
                    "{0} not a valid parameter. Must be one of {1}".format(
            ...
            if parameter_values[position] is not None:
                raise ValueError(
                    'Parameter {0} "{1}" was already set by positional argument.'.format(
                        position, name
                    )
                )
        ...
        for position, ((name, encoding), value) in enumerate(
            zip(self.parameters, parameter_values)
        ):
            if name is not None and value is None:
                raise ValueError(
                    'Parameter {0} "{1}" ({2}) was not set.'.format(
                        position, name, encoding
                    )
                )
    ...
    def parameter_items(self):
        return (
            (name, value)
            for ((name, encoding), value) in zip(
                self.parameters, self._parameter_values
            )
            if name is not None
        )
    ...
    def get_message_class_by_id(cls, message_id):
        message_classes = list(
            filter(lambda x: x.id == message_id, Message.__subclasses__())
        )
    ...
    def parse(cls, buffer):
        ...
        message_id, length, dest, source = base_package.unpack(
            buffer[:base_package_length]
        )
        ...
        if msg_cls.is_long_cmd:
            assert (
                descr["dest"] & 0x80
            ), "Long message expected, but message binary does not mark it as long."
            descr["dest"] &= 0x7F
    ...
    def __bytes__(self):
        ...
        if self.dest is None:
            raise RuntimeError(
                "Cannot convert message without destination " "to byte representation"
            )

        if self.source is None:
            raise RuntimeError(
                "Cannot convert message without source to " "to byte representation"
            )
