...
def find_stages(
    num_gpib_ports_check=10, gpib_device_nums_check=4, timeout_gpib_read_ms=100
):
    gpib_stages_found = []
...
class NewportStages(st.Stages2):
    def __init__(
        self,
        gpib_port_num,
        gpib_device_input_num,
        gpib_device_output_num,
        C1_input=None,
        C2_input=None,
        C1_output=None,
        C2_output=None,
        c1_c2_distance_mask_um=None,
        set_defaults_on_startup=False,
        update_position_absolute=100,
        centre_stage_and_set_home=False,
        x_axis_motor="x",
        y_axis_motor="y",
        z_axis_motor="z",
        filename=None,
...
class NewportSystem(object):
    Configuration = {
        ...
        "Query Echo": 2,
        "ASCII Command": 4,
        "Carriage Return Command Termination": 7,
        "Line Feed Command Termination": 8,
        "EOI Command": 9,
        "CR\LF": 10,
        "CR Commands CR\LF Response Termination": 11,
        "Hexadecimal": 12,
        "2nd Generation Serial Poll Bit Format": 13,
        "SRQ On Message": 14,
    }
    ...
    def _send_read_command(self, command, data=None):
        return NewportStage._send_read_command(
            self.gpib_stage, self.axis_str, command, data
        )
